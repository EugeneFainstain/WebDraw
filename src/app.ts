...
import { State, Event, Action } from './stateMachine';
...
function processDelete() {
    ...
    if (state.hasUndoableTransform && state.transformSnapshot && state.selectedStrokeIdx !== null) {
        ...
        const selectedStroke = state.strokeHistory[state.selectedStrokeIdx];
        const snapshot = state.transformSnapshot; // Local variable to avoid null checks
...
function handlePointerDown(e: PointerEvent) {
    ...
    if (state.eventHandler.getFingerCount() === 0) {
        ...
        if (state.firstTapUpTime > 0 &&
            ...
            getDistance(pos, state.firstTapDownPos) < DOUBLE_TAP_DISTANCE) {
...
function handlePointerUp(e: PointerEvent) {
    ...
    if (state.eventHandler.getFingerCount() === 0) {
        ...
        if (state.secondTapDownTime > 0 &&
            state.secondTapDownPos !== null &&
            getDistance(pos, state.secondTapDownPos) < DOUBLE_TAP_DISTANCE &&
            ...
            const result = state.cursorAnchor ? findClosestStrokeAndPoint(state.cursorAnchor) : null;
            if (result) {
                // Move cursor to the closest point
                state.cursorAnchor = result.point;
                // Select the stroke and store the point index
